...
在项目根目录下直接运行 `start.sh` 脚本即可零门槛启动：

```bash
# 1. 一键启动小说工坊（单端口生产集成模式，访问 http://127.0.0.1:8000）
./start.sh

# 2. 或以双端热更新模式启动（用于修改前端代码调试，访问 http://127.0.0.1:9000）
./start.sh dev

# 3. 常用运维管理命令
./start.sh status   # 查看服务运行状态、PID 与健康探针
./start.sh stop     # 一键停止后台服务
./start.sh logs     # 实时追踪运行日志
```
